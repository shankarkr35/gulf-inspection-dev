...
class Admin extends CI_Controller {
    ...
    function clientList()
    {
        $customers = $this->objcom->get_all_where('customers',$where="",$ord="id");
        if(!empty($customers)){
            foreach($customers as $key=>$cust){
                $company_id = $cust->company;
                $company_rec = $this->objcom->get_single_record('companies',array('nameUrl'=>$company_id));
                if(!empty($company_rec)){
                    $customers[$key]->company_name = $company_rec->name;
                }

            }
        }
        $data_array['customers'] = $customers;
        $this->load->view('ADMIN/include/header');
		$this->load->view('ADMIN/include/sidebar');
	...
	{
        $data_array['companies'] = $this->objcom->get_all_where('companies',$where="",$ord="id");
	    $this->load->view('ADMIN/include/header');
		$this->load->view('ADMIN/include/sidebar');
		$this->load->view('ADMIN/customers/add',$data_array);
		$this->load->view('ADMIN/include/footer');    
	}
	...
	function create_new_client()
	{
        ...
        $postData['company'] = $inputData['company_name'];
    ...
    function companies()
	{
	    $data_array['records'] = $this->objcom->get_all_where('companies',$where="",$ord="id");
	    $this->load->view('ADMIN/include/header');
		$this->load->view('ADMIN/include/sidebar');
		$this->load->view('ADMIN/companies/index',$data_array);
		$this->load->view('ADMIN/include/footer');    
	}
	...
	function createCompany()
	{
        ...
        $postData['create_date'] = date("Y-m-d H:i:s");
        $url = $postData['nameUrl'];
        $qry = $this->db->query("SELECT * FROM `companies` WHERE `nameUrl` = '$url'");
        $resData = $qry->row();
        if(empty($resData))
        {
            if($this->objcom->insert_data('companies',$postData))
            {
                $responseData['responseData'] = "new record inserted successfully";        
            }
        }else{
    ...
    {
        $data_array['record'] = $this->objcom->get_single_record('companies',array('id'=>$id));
        $this->load->view('ADMIN/include/header');
		$this->load->view('ADMIN/include/sidebar');
		$this->load->view('ADMIN/companies/edit',$data_array);
		$this->load->view('ADMIN/include/footer');        
    }
    ...
    function updateCompany()
    {
        ...
        $id = $inputData['id'];
        $url = $postData['nameUrl'];
        $qry = $this->db->query("SELECT * FROM `companies` WHERE `nameUrl` = '$url' AND `id`!='$id' ");
        $resData = $qry->row();
        if(empty($resData))
        {
            if($this->objcom->update_records('companies',$postData,$where=array('id'=>$id)))
            {
                $responseData['responseData'] = "record updated successfully";        
            }
        }else{
	...
	{
        $data_array['companies'] = $this->objcom->get_all_where('companies',$where="",$ord="id");
        $data_array['country'] = $this->objcom->get_all_where('country',$where="",$ord="id");
	...
	function createReport()
	{
        ...
        for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
            ...
            $qrWidth = 20; // Width of the QR code
            $qrHeight = 20; // Height of the QR code

            // Position the QR code at the top right corner
            $x = $size['width'] - $qrWidth - 10; // Margin from the right edge
            $y = 8;
        ...
        /*----- End ---------*/
        $rec = $this->objcom->get_all_where_limit('reports','','',$limit=1);
        //echo"<pre>";print_r($rec[0]->giico_job_serial);die;
        if(!empty($rec)){
            $giico_job_serial = str_pad((int)$rec[0]->giico_job_serial + 1, strlen($rec[0]->giico_job_serial), '0', STR_PAD_LEFT);

            //$giico_job_serial = $rec[0]->giico_job_serial + 1;
        }else{
            $giico_job_serial = '000001';
        }
        $giico_job_ref = 'GIICO'.'-'.date('Y').'-'.$giico_job_serial;
        //echo"<pre>";print_r($giico_job_serial);die;
        $postData['company'] = $inputData['company_name'];
        ...
        $postData['giico_job_ref'] = $giico_job_ref;
        $postData['giico_job_serial'] = $giico_job_serial;
        ...
        if(empty($resData))
        {
            ...
            if($report_id)
            {
                ...
                $hist_report['giico_job_ref'] = $giico_job_ref;
                $hist_report['update_times'] = 0;
                ...
                $companyData = $this->objcom->get_single_record('companies',array('nameUrl'=>$inputData['company_name']));
                if(!empty($companyData)){
                    $postData['company_name'] = $companyData->name;
                }else{
                    $postData['company_name'] = '';
                }
                $countryData = $this->objcom->get_single_record('country',array('id'=>$inputData['country']));
                if(!empty($companyData)){
                    $postData['country'] = $countryData->name;
                }else{
                    $postData['country'] = '';
                }
                $postData['giico_job_ref'] = $giico_job_ref;
    ...
    function updateReport()
    {
        ...
        $rec = $this->objcom->get_single_record('reports',array('id'=>$id));
        if(!empty($data_array)){
            $giico_job_serial = $data_array->giico_job_serial;
        }
        //$giico_job_ref = 'GIICO'.'-'.date('Y').'-'.$giico_job_serial;
        $postData['company'] = $inputData['company_name'];
    ...
    public function sendEmail($postData,$email)
    {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'qrsystem@giico.net';
        $mail->Password = 'Sys@giico24';
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;
                
        $mail->setFrom('qrsystem@giico.net', 'Gulf Inspection');
        //Add a recipient
    ...
    function get_contact_person()
    {
        ...
        $data_array['id'] = $this->input->post('id');
        $data_array['records'] = $this->objcom->get_all_where('customers',array('company'=>$company),"id");
        $html = $this->load->view('ADMIN/ajax/contact_person',$data_array,TRUE);
        $respone['response'] = $html;
        echo json_encode($respone);
    }
    ...
    public function company_import(){
        $data = array();
        $memData = array();
        // If import request is submitted
        if($this->input->post('importSubmit')){
                ...
                if(is_uploaded_file($_FILES['file']['tmp_name'])){
                    ...
                    if(!empty($csvData)){
                        foreach($csvData as $row){ $rowCount++;
                            ...
                            $prevCount = $this->objcom->getRows('companies',$con);
                            //echo"<pre>";print_r($prevCount);die;
                            if($prevCount > 0){
                                // Update member data
                                $condition = array('name' => $row['Name']);
                                $update = $this->objcom->update('companies', $memData, $condition);
                                ...
                                // Insert member data
                                $insert = $this->objcom->insert('companies', $memData);
